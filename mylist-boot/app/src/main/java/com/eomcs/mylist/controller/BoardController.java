...
public class BoardController {
  ...
  public BoardController() throws Exception {
    ...
    int c;
    while((c = in.read()) != -1) { // 파일에서 한 문자를 읽는다. 더이상 더이상 읽을 문자가 없다면 반복문을 종료한다. 

      if (c == '\n') { // 만약 읽은 문자가 줄바꿈 명령이라면 지금까지 읽은 csv데이터를 분석하여 Contact객체에 담는다. 
        boardList.add(Board.valueOf(buf.toString())); //파일에서 읽은 csv데이터로 객체를 초기화시킨후 목록에 등록한다. 
